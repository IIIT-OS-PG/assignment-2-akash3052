...
void* write1(void *threadarg)
{
	...
	int n;
	char buff[(512*1024)];
	memset(buff,'\0',(512*1024));//added new
	// rewind(fp);
	fseek(fp,0,SEEK_SET);
	fseek(fp,chunk_number*(512*1024),SEEK_SET);
	cout<<"ftell in write1 before writing :"<<ftell(fp)<<endl;
	// n=recv(newsockfd,buff,(512*1024),0);
	while((n=recv(newsockfd,buff,512,0))>0 && filesize>0)
	{
...
void* read1(void *threadarg)
{
	...
	int n;
	int size=(512*1024);
	// n=fread(buffer,sizeof(char),(512*1024),fp);
...
int main(int argc ,char *argv[])
{
	...
	if(s==1)
	{
		...
		int no_of_chunks=size1/(512*1024);
		if(size1%(512*1024)!=0)
			no_of_chunks++;
	...
	if(s==3)
	{
		...
		for(int i=0;i<no_of_clients;i++)
		{
			...
			cout<<"ip address :"<<ipadd<<"port_number :"<<port<<endl;
			if(connect(sockfd,(struct sockaddr *)&serveraddr,sizeof(serveraddr))<0)
			{
				cout<<"connetion failed"<<endl;
				// return -1;
